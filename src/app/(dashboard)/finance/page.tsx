...
export default function FinancePage() {
  return (
    <div className="min-h-screen bg-gray-50 p-6 lg:p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">정산 / 회계</h1>
        <p className="mt-1 text-sm text-gray-500">매출, 지출, 미수금 현황을 관리하고 정산을 처리합니다.</p>
      </div>

      {/* Summary Cards */}
      <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {summaryCards.map((card) => (
          <div key={card.label} className="rounded-xl bg-white border border-gray-200 shadow-sm p-6">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm text-gray-500">{card.label}</span>
              <span className="text-2xl">{card.icon}</span>
            </div>
            <p className="text-xl font-bold text-gray-900">{card.value}</p>
            <p className={`mt-1 text-sm font-medium ${card.positive ? 'text-green-600' : 'text-red-500'}`}>
              {card.change} 전월 대비
            </p>
          </div>
        ))}
      ...
      <div className="mb-6 flex gap-3">
        ...
        <button className="inline-flex items-center gap-2 rounded-lg bg-white border border-gray-300 px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
          <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="overflow-hidden rounded-xl bg-white border border-gray-200 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800">최근 거래 내역</h2>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-6 py-3.5 text-left font-semibold text-gray-700">내용</th>
                <th className="px-6 py-3.5 text-right font-semibold text-gray-700">금액</th>
                <th className="px-6 py-3.5 text-left font-semibold text-gray-700">구분</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {mockTransactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={`px-6 py-4 text-right font-semibold ${tx.type === '수입' ? 'text-green-600' : 'text-red-500'}`}>
                    {tx.type === '수입' ? '+' : '-'}{formatWon(tx.amount)}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      tx.type === '수입' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {tx.type}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-gray-700">{tx.manager}</td>
                </tr>
              ))}
            </tbody>
          </table>
